$(document).ready(function(){
	...
	var regexDesc = /^[A-Za-zÁáÉéÍíÓóÚúÜüÑñ'"-_,\.\d\s]{6,100}$/;
	var regexP1 = /^([\d]*\.[\d]{1,8})$/;
	var regexP2 = /^([\d]*)$/;
	var valCod = $('#id_codigo').val();
	var f_err = false;
	...
	$('#id_codigo').focus(function(){
		$(this).keyup(function(){
			...
			if ($(this).val().length >= 1 && $(this).val().match(regexCod) && $(this).val() !== 'PK-0000') {
				if (f_err === true && $(this).val() === valCod) {
					$('#d_codigo').addClass('has-error');
					$('#s_codigo').addClass('glyphicon glyphicon-remove');
					f_codigo = false;
				} else {
					$('#d_codigo').addClass('has-success');
					$('#s_codigo').addClass('glyphicon glyphicon-ok');
					f_codigo = true;
				}
			} else if ($(this).val() === '') {
				$('#d_codigo').removeClass('has-success has-error');
	...
	$('#id_descripcion').focus(function(){
		$(this).keyup(function(){
			...
			if ($(this).val().length >= 1 && $(this).val().match(regexDesc)) {
	...
	if (($('#id_precio_unitario').val().match(regexP1) || $(this).val().match(regexP2)) && ($('#id_precio_unitario').val() != "")) {
		$('#d_precio_unitario').addClass('has-success has-feedback');
		$('#s_precio_unitario').addClass('glyphicon glyphicon-ok form-control-feedback');
		f_precio_unitario = true;
	}
