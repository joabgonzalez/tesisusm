...
$(document).ready(function(){
	...
	var c = 0;
	$("#id_cantidad").on('focus', producto);
	$("#id_cantidad").on('focus', producto_inventario);
	...
	function producto_inventario(){
		...
		$.ajax({
			...
			success: function(data) {
				...
				c = c + 1;
				if (c == 1) {
					$('#d_cantidad').append('<p id="p_cantidad" class="help-block">La cantidad máxima disponible es: '+limit+'</p>');
				};
			},
			error: function(xhr,errmsg,err) {
				//console.log(xhr.status + ": " + xhr.responseText);
